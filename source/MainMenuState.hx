...
class MainMenuState extends MusicBeatState
{
	...
	override function create()
	{
		...
		var bg:FlxSprite = new FlxSprite(-80).loadGraphic(Paths.image('MenuSky'));
		...
		var yScroll:Float = Math.max(0.25 - (0.05 * (optionShit.length - 4)), 0.1);
		var building:FlxSprite = new FlxSprite(-150, -180).loadGraphic(Paths.image('MenuBuildings'));
		building.scrollFactor.set(0, yScroll);
		building.setGraphicSize(Std.int(building.width * 1.175));
		building.updateHitbox();
		building.antialiasing = ClientPrefs.globalAntialiasing;
		building.setGraphicSize(Std.int(building.width * 0.8));
		add(building);

		var bgside:FlxSprite = new FlxSprite(-80).loadGraphic(Paths.image('MenuRight'));
		bgside.scrollFactor.set(0, yScroll);
		bgside.updateHitbox();
		bgside.scale.set(1.1,1.1);
		bgside.screenCenter();
		bgside.x += 70;
		...
		if (optionShit[curSelected] == 'options')
			{
				...
				charrosa.dance();
				charrosa.updateHitbox();
				charrosa.visible = true;
			}
			else
				{
					charrosa.visible = false;
		...
		if (optionShit[curSelected] == 'freeplay')
			{
				...
				charrev.dance();
				charrev.updateHitbox();
				charrev.visible = true;
			}
			else
				{
					charrev.visible = false;
