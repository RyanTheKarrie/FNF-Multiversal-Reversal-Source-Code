	...
	override public function create()
	{
		...
		switch (curStage)
		{
			case 'stage': //Week 1
				...
				case 'roof': //ReynWeek
				var sky:BGSprite = new BGSprite('Reyn/roof/SkyBG', -600, -260, 0, 0);
				add(sky);

				var backbuilding0:BGSprite = new BGSprite('Reyn/roof/buildingback0', -600, -200, 0.6, 0.6);
				add(backbuilding0);

				var backbuilding:BGSprite = new BGSprite('Reyn/roof/buildingback', -600, -200, 0.7, 0.7);
				add(backbuilding);

				var frontbuilding:BGSprite = new BGSprite('Reyn/roof/Buildingfront', -600, -310, 0.8, 0.8);
				add(frontbuilding);

				phillyTrain = new BGSprite('Reyn/roof/thecar', 2000, 360);
				add(phillyTrain);
				...
				var roof:BGSprite = new BGSprite('Reyn/roof/TheFloor', -600, -320, 0.9, 0.9);
				roof.setGraphicSize(Std.int(roof.width * 1.1));
				roof.updateHitbox();
				add(roof);

			case 'thecrack': //ReynWeek
				var space:BGSprite = new BGSprite('Reyn/thecrack/reignskyNEW', -600, -260, 0, 0);
				add(space);

				var slash:BGSprite = new BGSprite('Reyn/thecrack/skyslash', -600, -500, 0.6, 0.6);
				add(slash);

				var cracks:BGSprite = new BGSprite('Reyn/thecrack/thecracks', -1000, -600, 0.7, 0.7);
				cracks.setGraphicSize(Std.int(cracks.width * 0.7));
				add(cracks);

				var buildings:BGSprite = new BGSprite('Reyn/thecrack/destroybuilding', -899, -125, 0.8, 0.8);
				buildings.setGraphicSize(Std.int(buildings.width * 0.7));
				add(buildings);

				var reyn:FlxSprite = new FlxSprite(319, -200);
						reyn.frames = Paths.getSparrowAtlas('Reyn/thecrack/reynTop');
						reyn.animation.addByPrefix('sigma', 'reyntop', 24, true);
						reyn.animation.play('sigma');
						reyn.setGraphicSize(Std.int(reyn.width * 0.5));
						reyn.scrollFactor.set(0.85, 0.85);
						reyn.antialiasing = true;
                        add (reyn);
			...
			case 'street': //song2
				var floor:BGSprite = new BGSprite('Reyn/street/thestreet', -600, -260, 0.9, 0.9);
				add(floor);
				if(!ClientPrefs.lowQuality) {
				var goku:FlxSprite = new FlxSprite(330, -150);
						goku.frames = Paths.getSparrowAtlas('Reyn/street/Goku');
						goku.animation.addByPrefix('sigma', 'Goku', 24, true);
						goku.animation.play('sigma');
						goku.setGraphicSize(Std.int(goku.width * 0.6));
						goku.scrollFactor.set(0.85, 0.85);
						goku.antialiasing = true;
                        add (goku);

				var siloo:FlxSprite = new FlxSprite(-550, -450);
						siloo.frames = Paths.getSparrowAtlas('Reyn/street/silooo');
						siloo.animation.addByPrefix('stare', 'THESHILOUTTE', 24, true);
						siloo.animation.play('stare');
						siloo.setGraphicSize(Std.int(siloo.width * 0.4));
						siloo.scrollFactor.set(0.85, 0.85);
						siloo.antialiasing = true;
                        add (siloo);

				var backcrowd:FlxSprite = new FlxSprite(120, -150);
						backcrowd.frames = Paths.getSparrowAtlas('Reyn/street/backcrowd');
						backcrowd.animation.addByPrefix('stare', 'bbackcrowd', 24, true);
						backcrowd.animation.play('stare');
						backcrowd.setGraphicSize(Std.int(backcrowd.width * 0.5));
						backcrowd.scrollFactor.set(0.85, 0.85);
						backcrowd.antialiasing = true;
                        add (backcrowd);

				var frontcrowd:FlxSprite = new FlxSprite(-1200, -650);
						frontcrowd.frames = Paths.getSparrowAtlas('Reyn/street/frontcrowd');
						frontcrowd.animation.addByPrefix('stare', 'FrontCrowd', 24, true);
						frontcrowd.animation.play('stare');
						frontcrowd.setGraphicSize(Std.int(frontcrowd.width * 0.3));
						frontcrowd.scrollFactor.set(0.85, 0.85);
						frontcrowd.antialiasing = true;
                        add (frontcrowd);
						

				var thedarkness:BGSprite = new BGSprite('Reyn/street/thedark', -600, -260, 0.9, 0.9);
						thedarkness.alpha = 0.8;
						
					if(!ClientPrefs.lowQuality) {		
				var raining :FlxSprite = new FlxSprite(-330, -350);
						raining.frames = Paths.getSparrowAtlas('Reyn/street/therainitself');
						raining.animation.addByPrefix('rain', 'therainitsself', 24, true);
						raining.animation.play('rain');
						raining.scrollFactor.set(0.85, 0.85);
						raining.antialiasing = true;
                        add (raining);
					}
				}

			case 'theshow': //ReynWeek
				var sky:BGSprite = new BGSprite('Reyn/TheShow/RosaBG', -900, -600, 0.5, 0.5);
				sky.setGraphicSize(Std.int(sky.width * 1.2));
				add(sky);

				var backstage:BGSprite = new BGSprite('Reyn/TheShow/StageBack', -600, -80, 0.8, 0.8);
				backstage.setGraphicSize(Std.int(backstage.width * 1.6));
				add(backstage);

				var crowdback:FlxSprite = new FlxSprite(-200, 300);
				crowdback.frames = Paths.getSparrowAtlas('Reyn/TheShow/backcrowd');
				crowdback.animation.addByPrefix('crowd', 'backcrowdcheer', 24, true);
				crowdback.animation.play('crowd');
				crowdback.setGraphicSize(Std.int(crowdback.width * 1.8));
				crowdback.scrollFactor.set(0.85, 0.85);
				crowdback.antialiasing = true;
				 add (crowdback);

				var roof:BGSprite = new BGSprite('Reyn/TheShow/StageFront', -1700, -900, 1, 1);
				roof.setGraphicSize(Std.int(roof.width * 1.6));
				roof.updateHitbox();
				add(roof);
		...
		if(curStage == 'street') {
			...
			var melo :FlxSprite = new FlxSprite(-600, 70);
						melo.frames = Paths.getSparrowAtlas('Reyn/street/themello');
						melo.animation.addByPrefix('melo', 'marshello', 24, true);
						melo.animation.play('melo');
						melo.setGraphicSize(Std.int(melo.width * 0.8));
						melo.scrollFactor.set(0.85, 0.85);
						melo.antialiasing = true;
                        add (melo);

			var raining :FlxSprite = new FlxSprite(-330, -150);
						raining.frames = Paths.getSparrowAtlas('Reyn/street/therainitself');
						raining.animation.addByPrefix('rain', 'therainitsself', 24, true);
						raining.animation.play('rain');
						raining.scrollFactor.set(0.85, 0.85);
			...
			var thedarkness:BGSprite = new BGSprite('Reyn/street/thedark', -600, -260, 0.9, 0.9);
						thedarkness.alpha = 0.5;
						add(thedarkness);
		}

		if(curStage == 'theshow') {

			var crowd:FlxSprite = new FlxSprite(-600, 550);
			crowd.frames = Paths.getSparrowAtlas('Reyn/TheShow/Rosalinecrowd');
			crowd.animation.addByPrefix('crowd', 'TheCrowd', 24, true);
			...
			var light:BGSprite = new BGSprite('Reyn/TheShow/StageBottomLight', -600, -260, 0.9, 0.9);
